...
def _plot_particle_ratios_vs_refmult(f, pids1, pids2):
    """
    ...
    """
    ratios = []
    for i, est_dir in enumerate(f.Sums[1:]):
        h3d = asrootpy(est_dir.FindObject("festi_pT_pid"))
        chist = asrootpy(f.results_post.correlations.Get("corr_hist_EtaLt05_vs_{}".format(est_dir.GetName())))
        try:
...
def _make_dNdeta_and_event_counter():
    with root_open(sys.argv[1], 'update') as f_post:
...
def _make_hists_vs_pt():
    ...
    with root_open(sys.argv[1], 'update') as f:
        ...
        dirname = 'results_post/pid_ratios'
        try:
...
def _make_dNdeta_andPNch_ratio_plots():
...
def _make_pid_ratio_plots():
    with root_open(sys.argv[1], 'update') as f:
        ...
        # Proton / pi_ch
        _plot_particle_ratios_vs_refmult(f, ['2212'], ['-211', '211'])
        # K / pi_ch
        _plot_particle_ratios_vs_refmult(f, ['310', '321', '-321'], ['-211', '211'])
        # Lambda / pi_ch
        _plot_particle_ratios_vs_refmult(f, ['3122'], ['-211', '211'])
        # Xi / pi_ch
        _plot_particle_ratios_vs_refmult(f, ['3322'], ['-211', '211'])
        # Omega / pi_ch
        _plot_particle_ratios_vs_refmult(f, ['3334', '-3334'], ['-211', '211'])

        # pi_ch/pi0
        _plot_particle_ratios_vs_refmult(f, ['-211', '211'], ['111',])
        # proton / pi0
        _plot_particle_ratios_vs_refmult(f, ['2212'], ['111',])
        # K / pi0
        _plot_particle_ratios_vs_refmult(f, ['310', '321', '-321'], ['111'])
        # Lambda / pi0
        _plot_particle_ratios_vs_refmult(f, ['3122'], ['111'])

        # Xi / pi0
        _plot_particle_ratios_vs_refmult(f, ['3322'], ['111'])
        # Omega / pi0
        _plot_particle_ratios_vs_refmult(f, ['3334', '-3334'], ['111'])
        # K0_S / K_ch
        _plot_particle_ratios_vs_refmult(f, ['310'], ['321', '-321'])

        # Currently not working, needs investigation!
        # K0_S / Lambda
        #_plot_particle_ratios_vs_refmult(f, ['310'], ['3122'])
        # K0_S / Xi
        #_plot_particle_ratios_vs_refmult(f, ['310'], ['3322'])
